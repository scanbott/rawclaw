---
name: clickup
description: Fetches all ClickUp tasks assigned to [CEO_NAME], presents status and priority, identifies which ones Cleo can act on (comms/outreach), drafts the proposed action for approval, executes on APPROVE, updates task status in ClickUp. Triggers on "/clickup", "check clickup", "what are my clickup tasks", "show me my tasks", "run through my to-dos".
user-invocable: true
---
...
Load env and pull all tasks assigned to [CEO_NAME] (user ID: [CEO_CLICKUP_USER_ID]) from Founders Kanban.
...
curl -s -X GET "https://api.clickup.com/api/v2/list/[CLICKUP_LIST_ID]/task?archived=false&include_closed=false&subtasks=true&assignees[]=[CEO_CLICKUP_USER_ID]&page=0" \
...
Only show tasks assigned to Chris (user ID [CEO_CLICKUP_USER_ID]) as "you". Still list others for context but flag them as team tasks.
...
1. Look up the contact in Obsidian knowledge vault if available: grep -r "[contact name]" ~/knowledge/
...
| Space | 901313716577 |
| Founders Kanban list | [CLICKUP_LIST_ID] |
| [CEO_NAME] user ID | [CEO_CLICKUP_USER_ID] |
