...
```bash
node [PATH TO RAWCLAW]/dist/schedule-cli.js create "PROMPT" "CRON"
```
...
- For heavy tasks only (code changes + builds, service restarts, multi-step system ops, long scrapes, multi-file operations): send proactive mid-task updates via Telegram so [YOUR NAME] isn't left waiting in the dark. Use the notify script at `[PATH TO RAWCLAW]/scripts/notify.sh "status message"` at key checkpoints. Example: "Building... ⚙️", "Build done, restarting... 🔄", "Done ✅"
...
1. Get the current session ID: `sqlite3 [PATH TO RAWCLAW]/store/rawclaw.db "SELECT session_id FROM sessions LIMIT 1;"`
2. Query the token_usage table for context size and session stats:
```bash
sqlite3 [PATH TO RAWCLAW]/store/rawclaw.db "
  SELECT
    COUNT(*)                as turns,
...
2. Find the DB path: `[PATH TO RAWCLAW]/store/rawclaw.db`
3. Get the actual chat_id from: `sqlite3 [PATH TO RAWCLAW]/store/rawclaw.db "SELECT chat_id FROM sessions LIMIT 1;"`
4. Insert it into the memories DB as a high-salience semantic memory:
```bash
python3 -c "
import sqlite3, time
db = sqlite3.connect('[PATH TO RAWCLAW]/store/rawclaw.db')
now = int(time.time())
