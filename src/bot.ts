...
async function handleMessage(ctx: Context, message: string, forceVoiceReply = false, skipLog = false): Promise<void> {
  ...
  try {
    ...
    if (errMsg.includes('exited with code 1')) {
      ...
      // Show the actual error so users can diagnose without digging through logs
      const shortErr = errMsg.length > 300 ? errMsg.slice(0, 300) + '...' : errMsg;
      await ctx.reply(`Something went wrong:\n\n${shortErr}\n\nTry /newchat or check the terminal logs.`);
    }
  }
}
...
export function createBot(): Bot {
  ...
  bot.command('dashboard', async (ctx) => {
    ...
    // Read DASHBOARD_URL fresh from .env each time (tunnel URL may have changed)
    const freshEnv = readEnvFile(['DASHBOARD_URL']);
    const base = freshEnv.DASHBOARD_URL || DASHBOARD_URL || `http://localhost:${DASHBOARD_PORT}`;
    const chatIdStr = ctx.chat!.id.toString();
    const url = `${base}/?token=${DASHBOARD_TOKEN}&chatId=${chatIdStr}`;
    // Send as both a clickable HTML link AND the raw URL so it always works
    await ctx.reply(
      `<a href="${url}">${url}</a>`,
  ...
  bot.command('skills', async (ctx) => {
    ...
    if (skills.length === 0) {
      await ctx.reply('No skills yet. Skills are auto-created from complex conversations.');
      return;
    }
    const lines = skills.map((s) => {
      const gen = s.autoGenerated ? ' (auto)' : '';
      return `• <b>${escapeHtml(s.name)}</b>${gen}\n  ${escapeHtml(s.description)}`;
    });
  ...
  bot.command('mcp', async (ctx) => {
    ...
    if (servers.length === 0) {
      await ctx.reply('No MCP servers configured. Add them to ~/.rawclaw/mcp.yaml');
      return;
    }
    const lines = servers.map((s) => {
      const status = s.enabled ? '✓' : '✗';
      return `${status} <b>${escapeHtml(s.name)}</b> (${s.type})`;
    });
    await ctx.reply(`MCP Servers:\n\n${lines.join('\n')}`, { parse_mode: 'HTML' });
  });

  // Text messages — and any slash commands not owned by this bot (skills, e.g. /todo /gmail)
  const OWN_COMMANDS = new Set(['/start', '/help', '/newchat', '/respin', '/voice', '/model', '/memory', '/forget', '/pin', '/unpin', '/chatid', '/wa', '/slack', '/dashboard', '/stop', '/agents', '/delegate', '/lock', '/status', '/skills', '/mcp']);
