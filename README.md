...
![RawClaw architecture](assets/architecture.jpeg)
...
![RawClaw memory system diagram](assets/memory-diagram.jpeg)

RawClaw has three layers of context working simultaneously, all automatic, nothing to configure.
...
"Remember that my project folder is ~/Projects/acme"
...
**Your environment** — File paths Claude should be able to reach without being told: project directories, anything you reference regularly.
...
- `todo` — read tasks from text files
...
RawClaw is designed to run on your personal machine for your own use. A few things to be aware of:
...
`cd rawclaw && git pull && npm install && npm run build` then restart. See [Updating RawClaw](#updating-rawclaw) above.
...
Each will show:
```
RawClaw agent [comms] online: @mark_comms_bot
```
...
Every agent inherits all of RawClaw's features with zero extra config:
