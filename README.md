...
╚██████╗███████╗██║  ██║╚███╔███╔╝
 ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝
  ...
  by Raw Growth  |  rawgrowth.ai
```

> **Your AI department in a box, delivered to your phone.**
> Built and maintained by [Raw Growth](https://rawgrowth.ai).
...
RawClaw v2 adds enterprise-grade infrastructure on top of the battle-tested v1 foundation. Zero new runtime dependencies -- everything is built with native APIs.

| Feature | What It Does |
|---------|-------------|
| **Supabase Dual-Write** | Optional cloud sync for heartbeat runs, budgets, audit logs. SQLite stays primary; Supabase is fire-and-forget secondary. Native `fetch()` client, no SDK. |
| **Budget Governance** | Per-agent spending limits (daily/monthly/lifetime). 80% warning threshold, auto-pause on exceed. Dashboard + Discord alerts. |
| **Heartbeat Execution** | Every agent run tracked: tokens, cost, duration, session state, exit code. Dead agent detection. Crash recovery with `resetStuckRuns()`. |
| **Activity Audit Log** | Immutable trail for all agent mutations. Actor-typed (user/agent/system), entity-scoped, JSON detail. In-memory buffer + SQLite + Supabase. |
| **Health Monitoring** | `/health` endpoint checks 5 subsystems (DB, Supabase, Telegram, memory, scheduler). Periodic self-check (5 min). Status change logging. |
| **Discord Adapter** | REST API notifications: budget alerts, health alerts, task completions. Webhook + Bot Token modes. Color-coded embeds. |
| **Plugin System** | YAML manifest plugins in `plugins/` directory. Event subscriptions, tool registration, lifecycle hooks. In-process TypeScript modules. |
| **Session Compaction** | Tracks per-agent session health. Auto-rotation at 200 runs / 2M tokens / 72h. Prevents context rot for long-running agents. |
| **Enhanced Dashboard** | 15+ new API endpoints for heartbeat, budget, activity, sessions, plugins, Discord, Supabase status. |
| **Enhanced Setup** | New wizard sections for Supabase, budget limits, Discord webhook configuration. |

### New Files (v2)

```
src/supabase.ts          -- Lightweight Supabase REST client
src/budget.ts            -- Budget governance with multi-scope policies
src/heartbeat.ts         -- Heartbeat execution model
src/audit.ts             -- Activity audit log (v2)
src/health.ts            -- Health check & monitoring
src/discord.ts           -- Discord REST adapter
src/plugins.ts           -- Plugin loader & event system
src/plugin-types.ts      -- Plugin type definitions
src/session-compaction.ts -- Session compaction tracking
```

### New SQLite Tables (v2)

```
heartbeat_runs       -- Every agent execution tracked
budget_policies      -- Per-agent spending limits
budget_incidents     -- Budget violation records
activity_log_v2      -- Rich audit trail
```
...
**Windows (PowerShell):**
```powershell
irm https://raw.githubusercontent.com/scanbott/rawclaw/main/install.ps1 | iex
```

The installer checks prerequisites, clones the repo, installs dependencies, and launches the setup wizard. The whole thing takes about 5 minutes.
...
```bash
git clone https://github.com/scanbott/rawclaw.git
cd rawclaw-v2.1
./setup
```

The setup script installs dependencies automatically, then launches the wizard.
