...
async function main() {
  console.log();
  console.log(`  ${c.bold}${c.cyan}BusinessOS Status${c.reset}`);
  ...
  if (process.platform === 'darwin') {
    try {
      const output = execSync('launchctl list com.[COMPANY_DOMAIN_REVERSED].app', {
        stdio: 'pipe',
      })
        .toString()
        .trim();
      ...
      if (pidMatch) {
        ...
        for (const line of lines) {
          ...
          if (parts.length >= 3 && parts[2] === 'com.[COMPANY_DOMAIN_REVERSED].app') {
            pid = parts[0].trim();
            break;
          }
        }
    ...
    try {
      const output = execSync('systemctl --user is-active businessos', {
        stdio: 'pipe',
      }).toString().trim();
      if (output === 'active') {
    ...
    try {
      execSync('pm2 describe businessos', { stdio: 'pipe' });
