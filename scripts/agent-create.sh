...
# Resolve config directory — BUSINESSOS_CONFIG from .env or environment, default ~/.businessos
if [ -z "$BUSINESSOS_CONFIG" ]; then
  BUSINESSOS_CONFIG=$(grep '^BUSINESSOS_CONFIG=' .env 2>/dev/null | cut -d'=' -f2- | sed "s|^~|$HOME|")
fi
BUSINESSOS_CONFIG="${BUSINESSOS_CONFIG:-$HOME/.businessos}"

echo "=== BusinessOS Agent Creator ==="
echo ""
...
# Config goes to BUSINESSOS_CONFIG if the dir exists, otherwise repo's agents/
if [ -d "$BUSINESSOS_CONFIG" ]; then
  AGENT_DIR="$BUSINESSOS_CONFIG/agents/$AGENT_ID"
...
echo "  3. Name it something like 'Mark $AGENT_LABEL' or 'BusinessOS $AGENT_LABEL'"
